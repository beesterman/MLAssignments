...
def hyperparamTesting():
    lossFunc = ["log_loss", "exponential"]
    learningRates = [.1, .01, .001]
    numOfEstimators = [50,100,150]

    inputFile = pd.read_csv(constants.basePath + "finalCsvs/DTTuning.csv", header=None)
    testCSV = open(constants.basePath + "finalCsvs/" + "DTBoostingTuning.csv", "w")
    testCSV.write("file,lossFunction,LearningRate,NumOfEstimators,accuracy,f1"+ "\n")
    ...
    for row in inputFile.itertuples(index=False):
        ...
        for lossFunction in lossFunc:
            for learningRate in learningRates:
                for numEstimators in numOfEstimators:
                    
                    classifier = GradientBoostingClassifier(loss=lossFunction, learning_rate=learningRate,n_estimators=numEstimators)
                    ...
                    if (f1 > bestF1):
                        bestF1 = f1
                        writeHold = [row[0], lossFunction, learningRate, numEstimators, accuracy, f1]
...
def testingOnData():
    ...
    inputFile = pd.read_csv(constants.basePath + "finalCsvs/DTBoostingTuning.csv", header=None)
    outfile = open(constants.basePath + "finalCsvs/DTBoostingTesting.csv", "w")
    outfile.write("file,lossFunction,LearningRate,NumOfEstimators,accuracy,f1"+ "\n")
    ...
    for row in inputFile.itertuples(index=False):
        ...
        classifier = GradientBoostingClassifier(loss=row[1], learning_rate=float(row[2]),n_estimators=int(row[3]))
        classifier.fit(trainData, trainLabels)    
